            ...
            <tr>
                <td>
                    ...
                    <div id="progressBox" style="display:<?php echo $autoStartCroppers ? 'block' : 'none'; ?>; margin-top:20px;">
                        <div style="font-weight:bold;margin-bottom:6px;">Building croppersÃ¢â‚¬Â¦</div>
                        ...
                        <div id="pinfo" style="margin-top:6px;font-size:13px;color:#333;">StartingÃ¢â‚¬Â¦</div>
						...
						<div id="autoNext" style="display:none;margin-top:10px;">
						  Proceeding to next step in <b id="cd">3</b>Ã¢â‚¬Â¦
						</div>
                    </div>
  ...
  function startCroppersBuild() {
    ...
    es.onmessage = function (e) {
      ...
      try {
        ...
        if (pct >= 100) {
          finished = true;
          plog.textContent += "Ã¢Å“â€¦ Completed!\n";
          plog.scrollTop = plog.scrollHeight;
          es.close();
          startCountdown();
        }
        ...
        if (d.error) {
          finished = true;
          plog.textContent += "Ã¢ÂÅ’ " + (d.msg || "Server reported an error.") + "\n";
          plog.scrollTop = plog.scrollHeight;
          es.close();
          startCountdown();
        }
    ...
    es.onerror = function () {
      // DonÃ¢â‚¬â„¢t spam after weÃ¢â‚¬â„¢re done
      if (finished) return;

      retries++;
      plog.textContent += "Ã¢Å¡Â  Connection hiccup (" + retries + "/" + MAX_RETRIES + "), retryingÃ¢â‚¬Â¦\n";
      plog.scrollTop = plog.scrollHeight;

      // EventSource will auto-reconnect by itself; we just decide when to give up
      if (retries >= MAX_RETRIES) {
        finished = true;
        plog.textContent += "Ã¢ÂÅ’ Too many connection failures Ã¢â‚¬â€ skipping croppers build.\n";
        plog.scrollTop = plog.scrollHeight;
        es.close();
