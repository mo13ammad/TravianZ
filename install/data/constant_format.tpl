...
##  Copyright:     nalooti (c) 2013-2025. All rights reserved.                 ##
    ...
    function config_env($key, $default = '')
    {
        ...
        if ($env === null) {
            ...
            foreach ($env as $name => $value) {
                $env[$name] = preg_replace_callback('/\$\{([A-Z0-9_]+)\}/i', function ($matches) use (&$env) {
                    $ref = $matches[1];
                    if (array_key_exists($ref, $env)) {
                        return $env[$ref];
                    }

                    $fromRuntime = getenv($ref);
                    return ($fromRuntime !== false) ? $fromRuntime : '';
                }, $value);
            }
        }
        ...
        return ($runtimeValue !== false) ? $runtimeValue : $default;
    }
}
...
define("SQL_SERVER", config_env("DB_HOST", config_env("DATABASE_HOST", "%SSERVER%")));

// ***** SQL Port
// default: 3306
define("SQL_PORT", (int) config_env("DB_PORT", config_env("DATABASE_PORT", %SPORT%)));

// ***** Database Username
define("SQL_USER", config_env("DB_USERNAME", config_env("DATABASE_USERNAME", config_env("MARIADB_USER", config_env("MYSQL_USER", "%SUSER%")))));

// ***** Database Password
define("SQL_PASS", config_env("DB_PASSWORD", config_env("DATABASE_PASSWORD", config_env("MARIADB_PASSWORD", config_env("MYSQL_PASSWORD", "%SPASS%")))));

// ***** Database Name
define("SQL_DB", config_env("DB_DATABASE", config_env("DATABASE_NAME", config_env("MARIADB_DATABASE", config_env("MYSQL_DATABASE", "%SDB%")))));
